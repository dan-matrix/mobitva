...
async function performGlobalSearch() {
    ...
    addResults(items, '📦 Предмет', i => `main.html?id=${i.id}&open=modal`);
    addResults(demons, '👹 Круг демона', i => `demon.html?id=${i.id}&open=modal`);
    addResults(totems, '🧪 Тотем', i => `totem.html?id=${i.id}&open=modal`);
    addResults(masterRunes, '🏰 Руна мастера', i => `master.html?id=${i.id}&open=modal`);
    addResults(druidsRunes, '🌿 Руна друидов', i => `druids.html?id=${i.id}&open=modal`);
    addResults(nakolki, '🏚️ Квестовая руна', i => `nakolki.html?id=${i.id}&open=modal`);
    addResults(news, '📰 Новость', i => `news.html?id=${i.id}&open=modal`);
    addResults(secretItems, '🔮 Секретная вещь', i => `secret_items.html?id=${i.id}&open=modal`);
    addResults(secretSets, '👘 Секретный сет', i => `secret_sets.html?set=${i.id}&open=modal`);
    addResults(mapLocations, '🗺️ Локация', i => `map.html?id=${i.id}&open=modal`);
